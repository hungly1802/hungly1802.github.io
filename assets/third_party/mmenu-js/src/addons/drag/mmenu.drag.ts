...
export default function(this: Mmenu) {
    ...
    if (options.menu.open) {
        this.bind('setPage:after', () => {
            ...
            var doPanstart = function(this: Mmenu, pos: number) {
                if (pos <= options.menu.maxStartPos) {
                    _stage = 1;
                }
            };
            var getSlideNodes = function(this: Mmenu): HTMLElement[] {
            ...
            switch (position) {
                ...
                case 'bottom':
                    drag.negative = true;
                    doPanstart = function(this: Mmenu, pos: number) {
                        if (
                            pos >=
            ...
            switch (zposition) {
                case 'front':
                    getSlideNodes = function(this: Mmenu): HTMLElement[] {
            ...
            _hammer.on('panend', evnt => {
                if (_stage == 2) {
                    ...
                    this[
                        _direction == drag.open_dir ? '_openStart' : 'close'
                    ]();
                }
                _stage = 0;
            });
